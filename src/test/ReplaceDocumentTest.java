...
public class ReplaceDocumentTest extends ServletTests{
    ...
    private void mockAnnotateAndClassify(Project project, ContractVersionInstance version, PortalUser user) throws BackOfficeException{
        ...
        new ContractAnnotation("", first, 1, "a comment", user, version, project, "", 0, now.getISODate()).store();

        new FragmentClassification(first, FeatureTypeTree.DEFINITION_CONCEPT.getName(), 0, 0, FragmentClassification.GENERATED, "", "", user, version, project, "", 0, 0, 70, "no rule specified", now.getISODate()).store();
